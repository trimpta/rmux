...
1. A Chrome window opens (maximized) at instagram.com/reels. Log in once — the session persists in `./profile/` between runs.
2. Once a reel page is showing, a small **Fork** pill appears in the bottom-left corner.
3. Click it: a new window opens tiled to the right playing that reel from the start. Your current window stays exactly where it was — scroll on whenever you're ready; focus stays where you clicked.
...
Every window is a real browser session: like, comment, scroll comments, mute — all native, nothing is automated.
